<template>
	<el-row>
		<el-col :span="24">
			...
			<div class="ui_msg">
				<div class="ui_icon_area"><i><img :src="imgSrc"/></i></div>
				<div class="ui_text_area">
					<h2 class="ui_msg_title">{{tips}}</h2>
				</div>
			</div>
		</el-col>
	</el-row>
...
<script>
	export default {
		...
		props:['msg'],
		mounted : function(){
			this.tips = this.msg || '您访问的页面不存在';
		},
		watch: {
			'msg': function(newVal){//监听路由变化
				this.tips = newVal || '您访问的页面不存在';
			}
		},
	}
</script>
...
.ui_msg .ui_text_area .ui_msg_title {
    margin-bottom: 15px;
    font-weight: 400;
    font-size: 16px;
}
